...
package com.netflix.spinnaker.clouddriver.api.v2.auth
...
class DockerBearerTokenService {
    ...
    public AuthenticateDetails parseBearerAuthenticateHeader(String header) {
        ...
        while (header.length() > 0) {
            ...
            if (keyEnd == -1) {
                throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Www-Authenticate header terminated with junk: '$header'.")
            }

            key = header.substring(0, keyEnd)
            header = header.substring(keyEnd + 1)
            if (header.length() == 0) {
                throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Www-Authenticate header unmatched parameter key: '$key'.")
            }
            ...
            if (header[0] == '"') {
                ...
                if (valueEnd == -1) {
                    throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException('Www-Authenticate header has unterminated " (quotation mark).')
                }
                ...
                if (header.length() != 0) {
                    if (header[0] != ",") {
                        throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Www-Authenticate header params must be separated by , (comma).")
                    }
                    header = header.substring(1)
                }
        ...
        if (!result.realm) {
            throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Www-Authenticate header must provide 'realm' parameter.")
        }
        if (!result.service) {
            throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Www-Authenticate header must provide 'service' parameter.")
        }
    ...
    public DockerBearerToken getToken(String repository, String authenticateHeader) {
        ...
        try {
            ...
            throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Failed to parse www-authenticate header: ${e.message}")
        }
        ...
        try {
            ...
            if (authWarning) {
                throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Authentication failed ($authWarning): ${e.getMessage()}", e)
            } else {
                throw new com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException("Authentication failed: ${e.getMessage()}", e)
            }
        }
    ...
    private interface TokenService {
        ...
        DockerBearerToken getToken(
                @Path(value = "path", encode = false) String path, @Query(value = "service") String service,
