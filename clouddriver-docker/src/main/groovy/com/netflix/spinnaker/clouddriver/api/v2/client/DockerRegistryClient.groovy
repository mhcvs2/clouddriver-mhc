...
package com.netflix.spinnaker.clouddriver.api.v2.client
...
import com.netflix.spinnaker.clouddriver.api.v2.auth.DockerBearerTokenService
import com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryAuthenticationException
import com.netflix.spinnaker.clouddriver.api.v2.exception.DockerRegistryOperationException
...
class DockerRegistryClient {
    ...
    interface DockerRegistryService {
        ...
        Response getTags(
                @Path(value = "repository", encode = false) String repository,
        ...
        Response getManifest(
                @Path(value = "name", encode = false) String name,
                @Path(value = "reference", encode = false) String reference,
                @Header("Authorization") String token, @Header("User-Agent") String agent)
        ...
        Response getCatalog(
                @Query(value = "n") int paginateSize,
                @Header("Authorization") String token, @Header("User-Agent") String agent)
